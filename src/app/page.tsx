...
<Image
      ...
      >
        ...
        <div
          ...
          //   タイムラインの幅を確保してほしい」／「縦長のときかなりひどい」）。
          //
          //   640px 固定だと、サイドバーが太った分だけ相対的に狭くなる。
          //   実測（1024px 幅・文字150%）: 列の右端が 997px まで押され、
          //   余白が 27px しか残らなかった。
          //
          //   ★ 縦長（820px 幅）では別の問題が出た。列が使える幅を全部使うので
          //     `mx-auto` に余白が残らず、**右端が画面に張り付く**（実測
          //     gapR=0）。左にはサイドバーがあるのに右には何もないので、
          //     見た目が大きく崩れる。
          //
          //   そこで「左右に最低限のガター」を確保する。ただし列自身が
          //   `px-6`（24px）の padding を持っているので、ガターを常に引くと
          //   **二重に引かれて列が狭くなる**（実測: 1440px で 690px 使えるのに
          //   658px になっていた）。
          //
          //   ガターが要るのは「列が使える幅をほぼ使い切る」ときだけ。
