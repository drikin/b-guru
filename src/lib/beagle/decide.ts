...
- **頻度は控えめに。next_activity_at はこの1回の行動から 25〜50分後に設定する（基本30分後程度）。** 絶対に10分以内にしない。
- **@ビーグル と明示的にメンションされた投稿（「== 明示的 @ビーグル ==」欄）は最優先で返信し、必ずその学習を learnings に含めよ。** 頻度の制約に関わらず、明示メンションへの返信は優先する。
